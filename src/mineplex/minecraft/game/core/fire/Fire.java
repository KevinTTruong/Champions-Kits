...
public class Fire extends MiniPlugin implements Serializable
{
	...
	public void IgniteCollide(UpdateEvent event)
	{
		...
		for (Item fire : _fire.keySet())
		{
			...
			for (LivingEntity ent : fire.getWorld().getEntitiesByClass(LivingEntity.class))
			{
				...
				if (ent.getLocation().getBlock().getType().equals(Material.WATER)
				||ent.getLocation().getBlock().getType().equals(Material.FIRE)
				||ent.getLocation().getBlock().getType().equals(Material.LAVA))
					continue;
	...
	public void IgnitePickup(PlayerPickupItemEvent event)
	{
		...
		||player.getLocation().getBlock().getType().equals(Material.FIRE)
