...
public class ClassManager extends MiniClientPlugin<ClientClass> implements IClassFactory, Serializable
{
	...
	*/
	...
	@Override
	public ClientClass AddPlayer(String player)
	{
		return new ClientClass(this, _skillFactory, _itemFactory, _clientManager.Get(player), null);
	}
	
	public ClientClass AddPlayer(String player, HashMap<Integer, CustomBuildToken> customBuildTokens)
	{
		return new ClientClass(this, _skillFactory, _itemFactory, _clientManager.Get(player), customBuildTokens);
	}
	...
	public void LoadCustomBuilds(HashMap<String, HashMap<String, HashMap<Integer, CustomBuildToken>>> data){
		for(String playerName:data.keySet()) {
			Get(playerName).LoadCustomBuilds(_classes.get(ClassType.Knight.name()), data.get(playerName).get("Knight"));
			Get(playerName).LoadCustomBuilds(_classes.get(ClassType.Brute.name()), data.get(playerName).get("Brute"));
			Get(playerName).LoadCustomBuilds(_classes.get(ClassType.Assassin.name()), data.get(playerName).get("Assassin"));
			Get(playerName).LoadCustomBuilds(_classes.get(ClassType.Ranger.name()), data.get(playerName).get("Ranger"));
			Get(playerName).LoadCustomBuilds(_classes.get(ClassType.Mage.name()), data.get(playerName).get("Mage"));
		}
	}
	
	public HashMap<String, List<CustomBuildToken>>> SaveCustomBuilds(){
		HashMap<String, HashMap<String, HashMap<Integer, CustomBuildToken>>> data = new HashMap<>();
		for(String playerName:GetKeys()) {
			data.put(playerName, Get(playerName).SaveCustomBuilds(_classes));
		}
		return data;
	}
}
